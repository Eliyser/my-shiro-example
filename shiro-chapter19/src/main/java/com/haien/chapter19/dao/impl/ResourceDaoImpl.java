package com.haien.chapter19.dao.impl;

import com.haien.chapter19.dao.ResourceDao;
import com.haien.chapter19.entity.Resource;
...
public class ResourceDaoImpl implements ResourceDao {
    ...
    public Resource createResource(final Resource resource) {
        final String sql = "insert into sys_resource(name, type, url, permission, parent_id, " +
                "parent_ids, available) values(?,?,?,?,?,?,?)";
    ...
    public Resource updateResource(Resource resource) {
        final String sql = "update sys_resource set name=?, type=?, url=?, permission=?, " +
                "parent_id=?, parent_ids=?, available=? where id=?";
        jdbcTemplate.update(
                sql,
    ...
    public Resource findOne(Long resourceId) {
        final String sql = "select id, name, type, url, permission, parent_id, parent_ids, " +
                "available from sys_resource where id=?";
    ...
    public List<Resource> findAll() {
        final String sql = "select id, name, type, url, permission, parent_id, parent_ids, " +
                "available from sys_resource order by concat(parent_ids, id) asc";
